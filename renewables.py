...
def REPD_date_corrected(year):
    #added year_ to allow for renewables to be curtailed from predicted
...
def add_marine_timeseries(year, year_baseline, scenario, time_step):
    ...
    #removing tidal stream so that it doesn't get removed by merge generators
    df_LOPF = pd.concat([df_LOPF, df_tidal_lagoon, df_wave_power], axis=1)
    ...
    df_tidal_stream.to_csv('LOPF_data/tstream.csv',header=True)
