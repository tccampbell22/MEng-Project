...
import renewables
from cleaning import unify_index
from cleaning import remove_double
...
def merge_generation_buses(year):
    ...
    all_gens=df_gen.index

    path = 'LOPF_data/generators-p_max_pu.csv'
    df_gen_p = pd.read_csv(path, index_col=0)

    carriers = ['Wind Offshore', 'Wind Onshore', 'Solar Photovoltaics', 'Large Hydro', 'Small Hydro', 'Interconnector', 'Tidal lagoon', 'Wave power']
    ...
    df_tidal_stream=pd.read_csv('LOPF_data/tstream.csv',index_col=0)
    
    columns_to_remove = [col for col in df_tidal_stream.columns if col not in all_gens]
    df_tidal_stream.drop(columns=columns_to_remove, inplace=True)
    
    df_gen_p=pd.concat([df_gen_p,df_tidal_stream],axis=1)
